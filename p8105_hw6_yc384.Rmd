...
bootstrap_sample %>% 
  ...
  ggplot() + 
  geom_histogram(aes(x = r.squared, y =..density.., alpha = 0.2, fill="")) +
  geom_density(aes(x = r.squared, y=..density.., alpha = 0.6)) +
  labs(
...
plot_r2 + 
  geom_vline(aes(xintercept = 0.8938942,  colour="red", linetype="dotted")) +
  geom_vline(aes(xintercept = 0.9278097,  colour="red", linetype="dotted")) +
    labs(
    title = "Distribution of Estimated RSquare",
    x = "Estimates of RSquare",
    y = "Density"
...
**Note:** From above plot, we can see that the distribution of R square is close to a normal distribution with a little heavy tail on the right.  The mean is a little over 0.91 and a 95% CI of (0.89, 0.93). This indicates that our model fits the data pretty well. 
...
```{r, message  = FALSE}
log_bs_sample %>% 
  ggplot() + geom_histogram(aes(x = log_b01, y =..density.., alpha = 0.2, fill = "red")) +   
  labs(
    title = "Distribution Plot of the Estimate of log(β̂0 ∗ β̂1)",
    x = "Estimate of log(β̂0 ∗ β̂1)",
    y = "Density",
    caption = "Data: 2017 Central Park weather data") +
  geom_density(aes(x = log_b01, y=..density.., alpha = 0.6)) +
...
**Note:** From the above plot, we can see that log(β̂0∗β̂1) follows a normal distributioasan as well with a heavier tail on the left. 
