...
**Note:** From above plot, we can see that the distribution of R square
is close to a normal distribution with a little heavy tail on the right.
The mean is a little over 0.91 and a 95% CI of (0.89, 0.93). This
indicates that our model fits the data pretty well.
...
**Note:** From the above plot, we can see that log(β̂0∗β̂1) follows a
normal distributioasan as well with a heavier tail on the left.
